...
namespace AGXUnity_Excavator.Scripts.Control.Execution
{
  ...
  public class ExcavatorCommandInterpreter
  {
    ...
    public OperatorCommand ConvertActuationToOperatorCommand( ExcavatorActuationCommand actuation )
    {
      ...
      switch ( m_joystickPattern ) {
        case ExcavatorJoystickPattern.SAE:
          command.LeftStickY = InverseScale( -actuation.Boom, m_boomScale );
          command.RightStickX = InverseScale( actuation.Bucket, m_bucketScale );
          command.RightStickY = InverseScale( actuation.Stick, m_stickScale );
          command.LeftStickX = InverseScale( actuation.Swing, m_swingScale );
          break;

        case ExcavatorJoystickPattern.ISO:
        default:
          command.RightStickY = InverseScale( -actuation.Boom, m_boomScale );
          command.RightStickX = InverseScale( actuation.Bucket, m_bucketScale );
          command.LeftStickY = InverseScale( actuation.Stick, m_stickScale );
          command.LeftStickX = InverseScale( actuation.Swing, m_swingScale );
          break;
      }
    ...
    private static float InverseScale( float value, float scale )
    {
      return Mathf.Abs( scale ) > 1.0e-5f ? value / scale : 0.0f;
    }
  }
}
