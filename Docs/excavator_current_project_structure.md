...
- `InterpolatedFollowAnchor.cs`
- `TrackedHmdPoseDriver.cs`

当前职责划分是：

- `ExperimentHUD` 继续负责桌面端 `OnGUI` 运行时信息面板
- `LinkCamera` 保留旧版单组件跟随相机逻辑，当前主要用于兼容 `AGXUnity_Excavator_measurements.unity` 等旧场景
- `TrackedCameraWindow` 继续负责 bucket / 机体等辅助相机窗口与 FPV 抓帧
- `InterpolatedFollowAnchor` 负责把桌面主相机父节点平滑跟随到驾驶舱/机器锚点，并在渲染前对低频源姿态做插值
- `TrackedHmdPoseDriver` 负责把 XR HMD 的本地 6DoF 位姿叠加到主相机子节点，避免把锚点跟随和头显姿态混在同一个组件里

需要特别注意：

- 当前 VR spectator mode 只落在 `AGXUnity_Excavator.unity`
- 主场景桌面 `Main Camera` 现已拆成“父锚点跟随 + 子相机 HMD pose”结构；旧 `LinkCamera` 不再挂在主场景 `Main Camera` 上
- 当前 HMD 预览直接复用桌面主相机链路，不引入单独的 XR spectator rig，也不修改 step-ack / ACT / teleop 控制链
