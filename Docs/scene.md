...
- the existing desktop `Main Camera` remains the only camera responsible for the
  desktop game window
- the desktop view keeps its HUD and auxiliary-window behavior, but the camera
  follow chain in `AGXUnity_Excavator.unity` is now split into a parent anchor
  (`InterpolatedFollowAnchor`) and a child camera pose driver
  (`TrackedHmdPoseDriver`)
- the old single-component `LinkCamera` path is kept only for legacy scenes such
  as `AGXUnity_Excavator_measurements.unity`
- the current PICO live-preview path mirrors the main desktop camera chain
  directly instead of creating a separate XR spectator rig in-scene
- HMD local pose is applied on the camera child, while machine/cab follow motion
  is applied on the parent anchor
- if live preview or XR startup fails, the project stays in pure desktop mode
  and the scene continues with desktop-only rendering
