...
namespace AGXUnity_Excavator.Scripts.Presentation
{
  public class PlannerDecisionVisualizer : MonoBehaviour
  {
    ...
    [SerializeField]
    [Min( 0.0f )]
    private float m_directionArrowHeightMeters = 0.09f;

    [SerializeField]
    [Min( 0.01f )]
    private float m_directionArrowHeadLengthMeters = 0.18f;

    [SerializeField]
    [Range( 5.0f, 85.0f )]
    private float m_directionArrowHeadAngleDegrees = 28.0f;
    ...
    [SerializeField]
    [Min( 0.0f )]
    private float m_actualBucketTipMarkerHeightMeters = 0.13f;

    [SerializeField]
    [Min( 0.01f )]
    private float m_actualBucketTipMarkerRadiusMeters = 0.08f;
    ...
    [SerializeField]
    private Color m_actualBucketTipMarkerColor = new Color( 1.0f, 0.05f, 0.95f, 0.95f );

    private LineRenderer m_selectedLine = null;
    private LineRenderer m_directionArrowLine = null;
    private LineRenderer m_directionArrowHeadLeft = null;
    private LineRenderer m_directionArrowHeadRight = null;
    private LineRenderer m_entryMarker = null;
    private LineRenderer m_exitMarker = null;
    private LineRenderer m_actualBucketTipMarkerX = null;
    private LineRenderer m_actualBucketTipMarkerZ = null;
    ...
    private void RefreshVisuals()
    {
      ...
      RefreshSelectedDirectionArrow( snapshot );
      RefreshActualBucketTipMarker( snapshot );
      RefreshDepletedCorridors( snapshot );
    }

    private void RefreshSelectedDirectionArrow( PlannerDebugSnapshot snapshot )
    {
      if ( !TryCorridorWorldPoints( snapshot.entry_x_m,
                                    snapshot.entry_z_m,
                                    snapshot.exit_x_m,
                                    snapshot.exit_z_m,
                                    m_directionArrowHeightMeters,
                                    out var entryWorld,
                                    out var exitWorld ) ) {
        HideSelectedDirectionArrow();
        return;
      }

      var direction = exitWorld - entryWorld;
      ...
      direction /= distance;
      SetLine( m_directionArrowLine,
               entryWorld,
               exitWorld,
               m_selectedColor,
               m_selectedWidth,
               true );
      SetMarker( m_exitMarker, exitWorld, m_exitMarkerColor, true );

      var headLength = Mathf.Min( m_directionArrowHeadLengthMeters, distance * 0.45f );
      var leftDirection = Quaternion.AngleAxis( m_directionArrowHeadAngleDegrees, Vector3.up ) * -direction;
      var rightDirection = Quaternion.AngleAxis( -m_directionArrowHeadAngleDegrees, Vector3.up ) * -direction;
      SetLine( m_directionArrowHeadLeft,
               exitWorld,
               exitWorld + leftDirection.normalized * headLength,
               m_selectedColor,
               m_selectedWidth,
               true );
      SetLine( m_directionArrowHeadRight,
               exitWorld,
               exitWorld + rightDirection.normalized * headLength,
               m_selectedColor,
               m_selectedWidth,
               true );
    }

    private void RefreshActualBucketTipMarker( PlannerDebugSnapshot snapshot )
    {
      if ( snapshot == null || !snapshot.current_bucket_tip_valid ) {
        HideActualBucketTipMarker();
        return;
      }

      var x = snapshot.current_bucket_tip_dig_area_x_m;
      var z = snapshot.current_bucket_tip_dig_area_z_m;
      var radius = Mathf.Max( 0.01f, m_actualBucketTipMarkerRadiusMeters );
      var height = m_actualBucketTipMarkerHeightMeters;
      if ( !TryDigAreaLocalPlanePointWorld( x - radius, z, height, out var leftWorld ) ||
           !TryDigAreaLocalPlanePointWorld( x + radius, z, height, out var rightWorld ) ||
           !TryDigAreaLocalPlanePointWorld( x, z - radius, height, out var backWorld ) ||
           !TryDigAreaLocalPlanePointWorld( x, z + radius, height, out var forwardWorld ) ) {
        HideActualBucketTipMarker();
        return;
      }

      SetLine( m_actualBucketTipMarkerX,
               leftWorld,
               rightWorld,
               m_actualBucketTipMarkerColor,
               Mathf.Max( 0.02f, m_selectedWidth * 0.75f ),
               true );
      SetLine( m_actualBucketTipMarkerZ,
               backWorld,
               forwardWorld,
               m_actualBucketTipMarkerColor,
               Mathf.Max( 0.02f, m_selectedWidth * 0.75f ),
    ...
    private bool TryDigAreaLocalPlanePointWorld( float x,
                                                 float z,
                                                 float heightMeters,
                                                 out Vector3 world )
    {
    ...
    private bool TryCorridorWorldPoints( float entryX,
                                         float entryZ,
                                         float exitX,
                                         float exitZ,
                                         float heightMeters,
    ...
    {
      entryWorld = Vector3.zero;
      exitWorld = Vector3.zero;
      return m_digAreaMeasurement != null &&
             m_digAreaMeasurement.TryDigAreaLocalPlanePointWorld(
               entryX,
               entryZ,
    ...
    private void EnsureRenderers()
    {
      ...
      m_directionArrowLine = GetOrCreateLineRenderer( "PlannerSelectedDirectionArrow", m_directionArrowLine, 43 );
      m_directionArrowHeadLeft = GetOrCreateLineRenderer( "PlannerSelectedDirectionArrowHeadLeft", m_directionArrowHeadLeft, 44 );
      m_directionArrowHeadRight = GetOrCreateLineRenderer( "PlannerSelectedDirectionArrowHeadRight", m_directionArrowHeadRight, 44 );
      ...
      m_actualBucketTipMarkerX = GetOrCreateLineRenderer( "PlannerActualBucketTipMarkerX", m_actualBucketTipMarkerX, 45 );
      m_actualBucketTipMarkerZ = GetOrCreateLineRenderer( "PlannerActualBucketTipMarkerZ", m_actualBucketTipMarkerZ, 45 );
    }
    ...
    private void HideSelectedDirectionArrow()
    {
      if ( m_directionArrowLine != null )
        m_directionArrowLine.enabled = false;
      if ( m_directionArrowHeadLeft != null )
        m_directionArrowHeadLeft.enabled = false;
      if ( m_directionArrowHeadRight != null )
        m_directionArrowHeadRight.enabled = false;
      if ( m_exitMarker != null )
        m_exitMarker.enabled = false;
    }

    private void HideActualBucketTipMarker()
    {
      if ( m_actualBucketTipMarkerX != null )
        m_actualBucketTipMarkerX.enabled = false;
      if ( m_actualBucketTipMarkerZ != null )
        m_actualBucketTipMarkerZ.enabled = false;
    }
  }
}
