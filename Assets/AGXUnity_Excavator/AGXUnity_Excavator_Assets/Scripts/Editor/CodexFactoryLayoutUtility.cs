...
namespace AGXUnity_Excavator.Scripts.Editor
{
  ...
  public static class CodexFactoryLayoutUtility
  {
    ...
    private const float RoomSizeX = CodexSceneScaleConfig.RoomSizeX;
    private const float RoomSizeZ = CodexSceneScaleConfig.RoomSizeZ;
    private const float RoomHeight = CodexSceneScaleConfig.RoomHeight;
    ...
    private const float ExcavatorCenterlineZ = CodexSceneScaleConfig.ExcavatorCenterlineZ;
    private const float ExcavatorBoomBaseX = CodexSceneScaleConfig.ExcavatorBoomBaseX;
    ...
    private static void BuildMeasuredFactoryLayout( string source )
    {
      ...
      try {
        ...
        result.message = $"Built measured factory layout from {source}. " +
                         $"Scale={CodexSceneScaleConfig.FormatScale( EnvironmentScale )}x; " +
                         $"room={RoomSizeX:0.###}m x {RoomSizeZ:0.###}m x {RoomHeight:0.###}m; " +
                         $"dump min={FormatVector2( DumpMin )}, dig min={FormatVector2( DigMin )}, " +
    ...
    private static string SaveCurrentSceneBackup( Scene scene )
    {
      ...
      var backupPath = Path.Combine( backupDirectory,
                                     "AGXUnity_Excavator_before_environment_" +
                                     CodexSceneScaleConfig.FormatScaleForFileName( EnvironmentScale ) + "x_" +
    ...
    private static void CreateAlignmentGuides( Transform parent, Vector3 digCenter, Vector3 dumpCenter, FactoryMaterials materials )
    {
      ...
      boomMarker.name = "Excavator_BoomBase_Target";
    ...
    private static void AlignExcavatorToMeasuredLayout( FactoryLayoutResult result )
    {
      ...
      result.warnings.Add( $"Excavator transform, size and internal AGX geometry were preserved; the {CodexSceneScaleConfig.FormatScale( EnvironmentScale )}x environment was built around the current machine pose." );
    }
