...
这份文档描述的是 `/home/pingfan/AGXUnityE85ExcavatorSim`
当前已经落地的项目结构，而不是早期设计草稿。历史资源目录和
C# namespace 仍保留 `AGXUnity_Excavator`，用于兼容现有场景引用。
...
- Unity 工程：`AGXUnityE85ExcavatorSim`
- `AGXUnity_Excavator.unity`
- 主挖机：Bobcat E85 (`Excavator_BobcatE85 Variant` / `Excavator_BobcatE85`)
- 场景环境倍率：`1.5`，由
  `AGXUnity_Excavator_Assets/Scripts/Editor/CodexSceneScaleConfig.cs`
  统一定义
...
- `2.0 kg` 及以下的 bucket 质量读数会按 `0.0 kg` 输出，用来屏蔽空斗姿态下的残留噪声
...
- `2.0 kg` 及以下的 bucket 质量读数会按 `0.0 kg` 输出
